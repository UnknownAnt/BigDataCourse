...
├── .gitignore                  # 🔒 Git 忽略规则
│
├── 实验一_数据生成与基础统计/     # M1: 数据生成与统计
├── 实验二_数据存储格式与性能对比/  # M1: 存储格式对比
├── 实验三_数据清洗与会话分析/     # M1: 数据清洗与会话
├── 实验四_ETL流水线与特征工程/    # M1: ETL与特征工程
├── 实验五_性能基准测试/           # M1: 性能基准测试
│
├── 实验六_数据流监听消费与背压机制/ # M2: 流监听与背压
├── 实验七_机器学习模型流式部署与实时打标/ # M2: ML流式打标
├── 实验八/                       # M2: 流处理管道统一入口
│   ├── run_pipeline.py          #   Producer + Consumer + ML预测
│   └── model.pkl                #   训练好的随机森林模型
│
├── 实验九_大模型API接入与非结构化特征提取/ # M3: LLM API接入
